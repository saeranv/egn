...
def diffusivity_coef(
    k:np.float64, rho:np.float64, c_p:np.float64
    ) -> np.float64:
...
def fourier_num(
    alpha:np.float64, char_len:np.float64, nt:np.float64
    ) -> np.float64:
...
def biot_num(
    h_c:np.float64, char_len:np.float64, k:np.float64
    ) -> np.float64:
...
def tau(delta_time:np.float64, t:np.float64)->np.float64:
...
def time_scale_nu(tau, Fo):
    """Time scale 2η^2, ratio of mass diffusion to forcing time scale.
...
def time_scale_chi(tau, Fo, Bi):
    """Time scale chi ξ - ratio of convection to forcing time scale.
...
def time_scale_epsilon(vol_z, rho_z, c_pz, area_m, char_len_m, rho_m, c_m):
    """Time scale epsilon ε, Cp of interior air to Cp of thermal mass.
